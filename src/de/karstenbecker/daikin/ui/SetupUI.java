...
public class SetupUI extends JPanel {
	...
	public SetupUI(DaikinPollingSettings settings) throws Exception {
		super(createLayout());
		daikin=new Daikin();
	...
	private void runDiscovery() throws Exception, IOException {
		List<String> endpoints = Files.readAllLines(new File("otherPotentialEndpoints.txt").toPath());
		Set<DaikinProperty> discoverProperties = daikin.discoverProperties(new InetSocketAddress(settings.getDaikinIP(), settings.getDaikinPort()), endpoints);
	...
	private static void createAndShowGUI() {
		...
		Type type = new TypeToken<List<DaikinProperty>>() {
		}.getType();
		String json;
		try {
			json = Files.readString(new File("discovered.json").toPath());
//			List<DaikinProperty> data = gson.fromJson(json, type);
			List<DaikinProperty> data =new ArrayList<DaikinProperty>();
			// Create and set up the content pane.
			SetupUI newContentPane = new SetupUI(new DaikinPollingSettings(data));
	...
	public String discoverIP() throws Exception {
		...
		return discoverDevice.getHostString();
	}
